...
export default async function VisionMaterielPage({params}: Props) {
    ...
    // Récupération des matériels liés
            ...
            {materiels && materiels.length > 0 ? (
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6">
                    {materiels.map((mat) => (
                        ...
                        >
                            <div
                                className="rounded-2xl p-4 text-white text-center font-bold flex flex-col items-center justify-center h-36 border border-white/10 bg-white/5 backdrop-blur-xl transition-transform active:scale-95">
                                <Image
                                    src={mat.icon}
                                    alt={mat.nom}
                                    width={56}
